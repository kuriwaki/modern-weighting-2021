...
```{r cbps, cache = TRUE}
fit_cbps <- CBPS(S ~ race + educ + state, data = frame_sel, ATT = 0, iterations = 100)
```

```{r ebal, cache = TRUE}
fit_ebal <- ebalance(Treatment = frame_sel$S, X = frame_sel_X)
```

```{r cbps-ebal}
par(mfrow = c(1, 2))
plot(x = frame_sel$weight[frame_sel$S == 1],
     y = 1 / fit_cbps$fitted.values[frame_sel$S == 1], 
     bty = "n",
     pch = 19,
     cex = 0.5,
     xlab = "CCES weights",
